...
#QD = pd.read_csv("Qdaily-10000x10_68_83.csv");
QD = np.array(QD)
...
MEF_thresholds = MEF_thresholds[:, 1]*0.3;
...
def deterministic(vars):
    ...
    for i in range(10):
        ...
        Q1[i] = QD[np.random.randint(0,999)]

    objectives = np.zeros((10, 4))

    num_cores = 14
    results = Parallel(n_jobs=num_cores)(delayed(eval_obj)(Q1[s], param) for s in range(10))
...
def eval_obj(Q1, param):
  ...
  for i in range(0, n_data):
   ...
   # print("---run_time %s seconds ---" % (time.time() - start_time))
    ...
    act_release_main_withSpill[i] = min(act_release_main[i] + spill[i]+MEF_releases[i],dis_cap_main);
    total_downstream_releases[i] = act_release_main[i] + spill[i]+MEF_releases[i];
